...
End-to-End (E2E) testing is a type of software testing that verifies an application’s complete workflow from start to finish, simulating real user scenarios to ensure that all integrated components work together as expected.
...
describe("Todos", () => {
  beforeEach(() => {
    cy.intercept(
      ...
      .intercept(
        ...
        { body: { text: "first todo", id: 1, isCompleted: false } }
      )
      .visit("/");
  });
  ...
  it("should update a todo", () => {
    cy.intercept(
      { method: "PUT", url: "http://localhost:3004/todos/1" },
      { body: { text: "bar", id: 1, isCompleted: true } }
    );
